...
export default function MealMenuPage() {
  ...
  async function submitFeedback(menuId, rating, comment) {
    try {
      await api.post(`food/daily-menu/${menuId}/feedback/`, {
        rating,
        comment
      });
  ...
  const dates = Object.keys(groupedWeekMenu).sort();
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap' }}>
          <div>
            ...
            <p style={{ opacity: 0.9, marginTop: '8px', fontSize: '1.1rem' }}>
              {isStudent ? "See what's cooking today!" : "Manage meal plans and grocery requirements"}
            </p>
          </div>
          {isStudent && (
            <div style={{ 
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '32px' }}>
        {todayMenu.length > 0 ? (
          ...
          <div className="card" style={{ textAlign: 'center', padding: '40px', gridColumn: '1 / -1' }}>
            <p style={{ fontSize: '1.2rem', color: 'var(--text-muted)' }}>No meals scheduled for today 🍽️</p>
          </div>
        )}
      </div>

      {/* Week Menu */}
      <h2 style={{ marginBottom: '16px' }}>Weekly Preview</h2>
          ...
          <div className="card" style={{ maxWidth: '400px', width: '90%', padding: '32px' }}>
              ...
              {[1, 2, 3, 4, 5].map((star) => (
                <button
                  ...
                  style={{
                    ...
                    border: 'none',
                    cursor: 'pointer',
                    opacity: feedback.rating >= star ? 1 : 0.3,
                    transition: 'all 0.2s'
                  }}
                >
                  ⭐
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              <button
                onClick={() => {
                  if (feedback.rating) {
                    submitFeedback(feedback.menuId, feedback.rating, feedback.comment || '');
                  } else {
